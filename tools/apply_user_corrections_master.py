#!/usr/bin/env python3
"""
Vox2Book — Master Fix Script for User Error Report
Executes comprehensive fixes across ALL 4094 paragraphs of "Диалоги_Анфи_и_Kir_2024-2026.docx":
1. Proper noun & brand capitalization (Сергей, Андрей, Анастасия, Мария, Анфия, Яндекс Маркет, ВКонтакте).
2. Spurious preposition "из-за" -> "из" (из старых фотографий, не из Google Play Market, из Telegram, из фильма, из чата, из мастерской, из Новгорода).
3. Hyphenated pronoun comma anomalies ("что, нибудь" -> "что-нибудь", "что, то" -> "что-то", "когда, то" -> "когда-то").
4. Specific word/STT garbles (вспорхнуть, посвящаю, нейросеть, бургер, закомплексованный, это у, всколыхнули).
5. Conjunction comma fixes ("А, когда" -> "А когда", "А, если" -> "А если").
6. Typography fixes (em-dashes, time spacing).
"""
...
DOC_PATH = Path(r"E:\coding\работа с литературой\output\books\Диалоги_Анфи_и_Kir_2024-2026.docx")
...
def fix_paragraph_text(text: str) -> str:
    ...
    # General typo & whitespace cleanup
...
def apply_master_corrections():
    ...
    print("Applying Master User Corrections across ALL pages...")
    ...
    doc_new.save(str(DOC_PATH))
    print(f"[Done Master Fixes] All user-reported error patterns successfully fixed!")
    print(f"  - Messages processed: {msg_count}")
    print(f"  - Messages with applied corrections: {fix_count}")
    print(f"  - Headers/Date lines: {header_count}")

if __name__ == "__main__":
    apply_master_corrections()
